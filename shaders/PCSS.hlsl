...
//生成随机数的函数，输入为一个二维向量，输出为一个0到1之间的随机数
float Random(float2 seed)
{
    return frac(sin(dot(seed, float2(12.9898, 78.233))) * 43758.5453);
}
// 生成 Vogel Disk采样点，输入为一个二维向量和采样点数量，输出为一个二维数组，包含了采样点的偏移量
...
// 生成 Poisson Disk采样点，输入为一个二维向量和采样点数量，输出为一个二维数组，包含了采样点的偏移量
...
// https://ruochenhua.github.io/2024/12/08/soft-shadow/
float FindBlocker(Texture2DArray cascadeShadowMap, int cascadeIndex, float2 uv, float depth)
{
    ...
    for (int i = 0; i < g_ShadowData.NumSamples; ++i)
    {
        ...
        //Texture2DArray（纹理数组）的采样方式
        //exture2DArray：采样时需要的是一个 float3​ 类型的坐标，其中：
        //.xy分量 (前两个)：就是常规的 float2UV 坐标，用于在单个纹理切片内定位。
        //.z分量 (第三个)：是一个数组索引，用于指定要采样哪一个纹理切片（即哪一级级联）。
...
float PCF(Texture2DArray cascadeShadowMap, int cascadeIndex, float2 uv, float depth, float filterRadius, int sampleCount)
{
    ...
    for (int i = 0; i < sampleCount; i++)
    {
        ...
        /*
        1. cascadeShadowMap.SampleCmpLevelZero(...)
        这是 HLSL 专用的阴影比较采样函数
        SampleCmp​ = "Sample with Comparison"（带比较的采样）
        它同时做两件事：采样深度值 + 与参考值比较
        普通采样：Sample()→ 返回纹理颜色值
        比较采样：SampleCmp()→ 返回比较结果（0或1）
        LevelZero​ = 强制使用 Mipmap 的第0级（最高分辨率）
        阴影计算需要精确，不能使用自动选择的低分辨率Mip级别
        */
...
float PCSS(Texture2DArray cascadeShadowMap, int cascadeIndex, float3 positionV)
{
    ...
    if (avgBlockerDepth > 0)
    {
        ...
        //loat penumbraRatio = (depth - avgBlockerDepth) * LIGHT_SIZE / avgBlockerDepth;
